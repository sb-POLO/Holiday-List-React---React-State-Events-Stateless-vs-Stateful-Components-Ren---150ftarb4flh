...
const lists = cityList.filter((item) => {
  if (item.name === 'Goa' || item.name === 'Amsterdam' || item.name === 'New York' || item.name === 'Darjeeling' || item.name === 'Tokyo' || item.name === 'Lonavala')
    return true;
  else
    return false;
})
...
const App = () => {
  ...
  let arr = lists.map((item) => {
    ...
    return <li key={"location" + (lists.indexOf(item) + 1)}>{item.name}({item.country})</li>
  });
